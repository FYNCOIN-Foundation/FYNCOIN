...
VERSION=
commit=false
url=https://github.com/fynproject/fyn
proc=2
mem=2000
lxc=true
...
Run this script from the directory containing the fyn, gitian-builder, gitian.sigs, and fyn-detached-sigs.
...
-u|--url	Specify the URL of the repository. Default is https://github.com/fynproject/fyn
...
then
	...
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit fyn=${COMMIT} --url fyn=${url} ../fyn/contrib/gitian-descriptors/gitian-linux.yml
	    ./bin/gsign --signer $SIGNER --release ${VERSION}-linux --destination ../gitian.sigs/ ../fyn/contrib/gitian-descriptors/gitian-linux.yml
	    mv build/out/fyn-*.tar.gz build/out/src/fyn-*.tar.gz ../fyn-binaries/${VERSION}
	fi
	# Windows
	if [[ $windows = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit fyn=${COMMIT} --url fyn=${url} ../fyn/contrib/gitian-descriptors/gitian-win.yml
	    ...
	    mv build/out/fyn-*.zip build/out/fyn-*.exe ../fyn-binaries/${VERSION}
	fi
	# Mac OSX
	if [[ $osx = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit fyn=${COMMIT} --url fyn=${url} ../fyn/contrib/gitian-descriptors/gitian-osx.yml
	    ...
	    mv build/out/fyn-*.tar.gz build/out/fyn-*.dmg ../fyn-binaries/${VERSION}
	fi
	# AArch64
	if [[ $aarch64 = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit fyn=${COMMIT} --url fyn=${url} ../fyn/contrib/gitian-descriptors/gitian-aarch64.yml
	    ./bin/gsign --signer $SIGNER --release ${VERSION}-aarch64 --destination ../gitian.sigs/ ../fyn/contrib/gitian-descriptors/gitian-aarch64.yml
	    mv build/out/fyn-*.tar.gz build/out/src/fyn-*.tar.gz ../fyn-binaries/${VERSION}
	fi
	popd || exit
...
then
	...
	then
	    ...
	    echo ""
	    ./bin/gbuild -i --commit signature=${COMMIT} ../fyn/contrib/gitian-descriptors/gitian-win-signer.yml
	    ...
	    mv build/out/fyn-*win64-setup.exe ../fyn-binaries/${VERSION}
	    mv build/out/fyn-*win32-setup.exe ../fyn-binaries/${VERSION}
	fi
	# Sign Mac OSX
	if [[ $osx = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -i --commit signature=${COMMIT} ../fyn/contrib/gitian-descriptors/gitian-osx-signer.yml
	    ...
	    mv build/out/fyn-osx-signed.dmg ../fyn-binaries/${VERSION}/fyn-${VERSION}-osx.dmg
	fi
	popd || exit
