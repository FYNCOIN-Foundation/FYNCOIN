...
CzFYNWallet::CzFYNWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zfyn seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZFYNSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZFYNSeed_deprecated()) {
                    LogPrintf("%s: Updated zFYN seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zfyn seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zfyn wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzFYNWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZFYNCount(nCountLastUsed);
    else if (!walletdb.ReadZFYNCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzFYNWallet::Lock()
{
    seedMaster = 0;
}

void CzFYNWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzFYNWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZFYN(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzFYNWallet::LoadMintPoolFromDB()
{
...
void CzFYNWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzFYNWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzFYNWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zfynTracker->HasPubcoinHash(pMint.first)) {
...
bool CzFYNWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZFYN(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zfynTracker which also adds to database
    pwalletMain->zfynTracker->Add(dMint, true);
...
void CzFYNWallet::SeedToZFYN(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzFYNWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzFYNWallet::GenerateDeterministicZFYN(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzFYNWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzFYNWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
