...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 FYN at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zFYN send
enum ZerocoinSpendStatus {
    ZFYN_SPEND_OKAY = 0,                            // No error
    ZFYN_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZFYN_WALLET_LOCKED = 2,                         // Wallet was locked
    ZFYN_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZFYN_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZFYN_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZFYN_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZFYN_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZFYN_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZFYN_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZFYN_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZFYN_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZFYN_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZFYN_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZFYN_SPENT_USED_ZFYN = 14,                      // Coin has already been spend
    ZFYN_TX_TOO_LARGE = 15,                         // The transaction is larger than the max tx size
    ZFYN_SPEND_V1_SEC_LEVEL
};
...
public:
    ...
    bool CreateZFYNOutput(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    string GetUniqueWalletBackupName(bool fzfynAuto) const;
    ...
    */
    ...
    CzFYNWallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<CzFYNTracker> zfynTracker;
    ...
    void setZWallet(CzFYNWallet* zwallet)
    {
        zwalletMain = zwallet;
        zfynTracker = std::unique_ptr<CzFYNTracker>(new CzFYNTracker(strWalletFile));
    }

    CzFYNWallet* getZWallet() { return zwalletMain; }
