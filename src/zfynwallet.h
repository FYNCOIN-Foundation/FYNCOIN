...
#ifndef FYN_ZFYNWALLET_H
#define FYN_ZFYNWALLET_H
...
public:
    CzFYNWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZFYN(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZFYN(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //FYN_ZFYNWALLET_H
