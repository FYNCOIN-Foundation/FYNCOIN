...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::FYN);
    ...
    if (!settings.contains("nAnonymizeFYNAmount"))
        settings.setValue("nAnonymizeFYNAmount", 1000);

    nAnonymizeFYNAmount = settings.value("nAnonymizeFYNAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeFYNAmount"))
        SoftSetArg("-anonymizefynamount", settings.value("nAnonymizeFYNAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in fyn.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeFYNAmount:
            return QVariant(nAnonymizeFYNAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeFYNAmount:
            nAnonymizeFYNAmount = value.toInt();
            settings.setValue("nAnonymizeFYNAmount", nAnonymizeFYNAmount);
            emit anonymizeFYNAmountChanged(nAnonymizeFYNAmount);
            break;
        case CoinControlFeatures:
