...
public:
    ...
    bool WriteZFYNSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZFYNSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZFYNSeed_deprecated(uint256& seed);
    bool EraseZFYNSeed();
    bool EraseZFYNSeed_deprecated();

    bool WriteZFYNCount(const uint32_t& nCount);
    bool ReadZFYNCount(uint32_t& nCount);
